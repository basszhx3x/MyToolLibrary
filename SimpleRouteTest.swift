...
class SimpleRouteTest {
    ...
    func runTests() {
        ...
        let testCases: [TestCase] = [
            // 基本路径测试
            TestCase(registeredPath: "home", requestPath: "/home", expectedMatch: true, expectedParams: [:]),
            // 带参数路径测试
            TestCase(registeredPath: "detail/:id", requestPath: "/detail/123", expectedMatch: true, expectedParams: ["id": "123"]),
            // 复杂参数路径测试
            TestCase(registeredPath: "product/:productId/info", requestPath: "/product/456/info", expectedMatch: true, expectedParams: ["productId": "456"]),
            // 不匹配路径测试
            TestCase(registeredPath: "detail/:id", requestPath: "/home", expectedMatch: false, expectedParams: nil),
            // 参数数量不匹配测试
            TestCase(registeredPath: "detail/:id", requestPath: "/detail/123/extra", expectedMatch: false, expectedParams: nil),
        ]
        ...
        for (index, testCase) in testCases.enumerated() {
            ...
            if matchResult == testCase.expectedMatch {
                if matchResult == true {
                    // 检查参数是否正确
                    if let expectedParams = testCase.expectedParams, parameters == expectedParams {
                        print("✅ 测试通过")
                        passedTests += 1
                    } else {
                        print("❌ 测试失败：参数不匹配")
